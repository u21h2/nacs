...
// CheckMultiPoc 检查一个url对应的多个poc workers是poc的数量
...
// executePoc 实际进行单个poc检查 返回是否存在漏洞
	...
	for _, rule := range p.Rules {
		...
		look1:
			...
			for _, var1 := range p.Sets[varset[0]] {
				...
				if success == true {
					logger.Success(fmt.Sprintf("%s://%s%s %s", req.Url.Scheme, req.Url.Host, req.Url.Path, var1))
					break look1
				}
			}
			if success == false {
		...
		look2:
			...
			for _, var1 := range p.Sets[varset[0]] { //username
				for _, var2 := range p.Sets[varset[1]] { //password
					...
					if success == true {
						logger.Success(fmt.Sprintf("%s://%s%s %s %s", req.Url.Scheme, req.Url.Host, req.Url.Path, var1, var2))
						break look2
					}
				}
			}
			...
			for _, var1 := range p.Sets[keys[0]] {
				for _, var2 := range p.Sets[keys[1]] {
					for _, var3 := range p.Sets[keys[2]] {
						...
						if success == true {
							logger.Success(fmt.Sprintf("%s://%s%s %s %s %s", req.Url.Scheme, req.Url.Host, req.Url.Path, var1, var2, var3))
							break look3
						}
					}
				}
